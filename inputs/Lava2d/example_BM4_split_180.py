...
sim.set_topo_incline(
    ...
    length_downstream = 1.0,          # meters (vent to downstream edge)
    width = 1.5,                      # meters (y-direction)
    dx = 0.006,                        # 1 cm grid spacing
    dy = 0.006
    )
...
#     arm_length = 0.20,                # meters (14 cm per side, as in experiment)
...
# Example usage on an inclined plane:

sim.set_topo_flat_obstacle(
    height=0.5,              # meters, obstacle height
    length_x=0.03,           # meters, extent in flow direction (x)
    width_y=.30,             # meters, width perpendicular to flow (y)
    center_x=0.25,           # meters, position on incline
    center_y=0.0,            # meters, centered on flow path
    add_to_existing=True     # add on top of the incline
)

# sim.export_dem('topography_flat_obstacle.asc', format='ascii')

# Export topography for visualization
sim.export_dem('topography_incline_triangle_180.asc', format='ascii')
...
sim.set_vent_simple(
    ...
    vent_width = 0.04,                # 6 cm fissure width
    discharge = 3.33e-5,             # m³/s (DRE)
    duration_hr = 0.2,                # 6 minutes
...
sim.set_runtime(
    ...
    max_time_hr = 0.2,                # 0.1 hours = 6 minutes
    out_times = [0.04, 0.08, 0.12, 0.16, 0.20],  # Output times (hours)
...
#===============================================================================
sim.set_output(
    path_out = './outputs/split_180/'            # Output directory
    )
